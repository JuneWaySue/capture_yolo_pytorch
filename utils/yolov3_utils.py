...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4, cls_conf_thres=0.0):
    """
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        detections = detections[detections[:,5] >= cls_conf_thres]
...
def evaluate(model, iou_thres, conf_thres, nms_thres, img_size, data_path, batch_size, device):
    model.eval()

    # Get dataloader
    dataset = myDataset(img_size,data_path,augment=False,multiscale=False,is_train=False)
    ...
    losses = {'YOLO Layer 0':yolo0_loss.mean(),'YOLO Layer 1':yolo1_loss.mean(),'YOLO Layer 2':yolo2_loss.mean()}

    return precision, recall, AP, f1, ap_class, losses